...
import {getAuthUserData} from "@/context/DashboardContext.jsx";
...
export default function HeaderDropdown() {
    ...
    // Profil fotoğrafı veya initial render
    const renderAvatar = () => {
        if (isLoading) {
            return (
        ...
        if (photo) {
            ...
            return (
                <div className="w-8 h-8 rounded-2xl bg-gradient-to-r from-[#667eea] to-[#764ba2] flex items-center justify-center border-2 border-white/20">
        ...
        // Fotoğraf yoksa ismin ilk harfi
        if (username) {
            return (
                <div className="w-8 h-8 rounded-2xl bg-gradient-to-r from-[#667eea] to-[#764ba2] flex items-center justify-center border-2 border-white/20">
                    <span className="text-white font-semibold text-sm">
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                onClick={() => setOpen((v) => !v)}
                className="flex items-center space-x-3 rounded-lg p-3 text-white bg-gradient-to-br from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl"
            >
                {renderAvatar()}
